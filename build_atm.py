    ...
    def ini_y(self, data_var, data_atm): 
        ...
        if vulcan_cfg.use_condense == True:
            for sp in vulcan_cfg.condesne_sp:
                ...
                print ( sp + " condensed from nz = " + str(conden_lev))
        ...
        data_var.y_time.append(y_ini)
        data_var.t_time.append(0)
...
class Atm(object):
    ...
    def __init__(self):
        self.Tco = vulcan_cfg.T_box
        self.pco = vulcan_cfg.p_box
    ...
    def load_TPK(self, data_atm, output):
        ...
        data_atm.Tco = np.array([vulcan_cfg.T_box])
        data_atm.pco = np.array([vulcan_cfg.p_box])
        
        # calculating and storing M(the third body)
        data_atm.M = data_atm.pco/(kb*data_atm.Tco)
        data_atm.n_0 = data_atm.M
