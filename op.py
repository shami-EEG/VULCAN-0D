...
class ReadRate(object):
    ...
    def __init__(self):
        ...
        # for efficiency of three-body reactions
        self.atm_base = vulcan_cfg.atm_base
        ...
        with open(vulcan_cfg.network) as f:
            for line in f.readlines():
                ...
                if line.startswith("# special"): 
                    special_re = True # switching to reactions with special forms (hard coded) 
                ...
                elif line.startswith("effi"):
                    re_i = i-2 # because i already + 2  
                    effi = "".join(line.split()[1:])
                    var.effic[re_i] = ast.literal_eval(effi)
                    #print var.effic[re_i][self.atm_base]
                    k[re_i] *=  var.effic[re_i][self.atm_base]
...
class Integration(object):
    ...
    def __init__(self, odesolver, output):
        ...
        self.y_time_freq = vulcan_cfg.y_time_freq
    ...
    def conv(self, var, para, atm, out=False, print_freq=100):
        ...
        if indx == para.count or indx == para.count-1: indx-=10  #Important!! For dt larger than half of the runtime (count-1 is the last one) 
        longdy = np.abs( (y_time[count-1] - y_time[indx])/atm.n_0 )
    ...
    def stop(self, var, para, atm):
        ...
        This function is 
        '''
        if para.count == 0: return False
        
        elif var.t > vulcan_cfg.trun_min and para.count > vulcan_cfg.count_min and self.conv(var, para, atm):
    ...
    def conden(self, var, atm):
        ...
        for re in var.conden_re_list:
            if var.Rf[re] == 'H2O -> H2O_l_s':
                m = 18./Navo
                rho_p = 1.
                r_p = 1.e-5 # assuming 0.1 micron from Table 2. [Toon & Farlow]
...
class ODESolver(object):
    ...
    def lhs_jac_0D(self, var, atm):      
        """
        directly constructing lhs = 1./(r*h)*sparse.identity(ni*nz) - dfdy for 0D box solver
        jacobian matrix for dn/dt + dphi/dz = P - L (including molecular diffusion)
        """
        y = var.y.copy()
        # TEST condensation excluding non-gaseous species
        if vulcan_cfg.use_condense == True:
            ysum = np.sum(y[atm.exc_conden])
        else: ysum = np.sum(y)
        # TEST condensation excluding non-gaseous species
...
class Ros2(ODESolver):
    ...
    def store_bandM_1D(self, a, nb, nn):
        """
    ...
    def solver(self, var, atm, para):
        ...
        # TEST condensation excluding non-gaseous species
        if vulcan_cfg.use_condense == True:
            var.ymix = var.y/np.vstack(np.sum(var.y[atm.exc_conden])) 
    ...
    def solver_fixbot(self, var, atm, para):
        ...
        if vulcan_cfg.use_moldiff == True:
            diffdf = self.diffdf
            jac_tot = self.lhs_jac_tot_fixbot
        else:
...
#                 delta[:,species.index(sp)] = 0
    ...
    def old_solver_fixbot(self, var, atm, para):
        """
        2nd order Rosenbrock [Verwer et al. 1997] with banded-matrix solver
